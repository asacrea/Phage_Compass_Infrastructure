...
def main(argv):
     ...
     input_f = "/Users/andressacre/Documents/Universidad/Investigación_Thomas/Code/Intrinsic-Dimension/pivote_nucleotide_dereplicated.tsv"
     me=args.metric
     n_neighbors = 3
     radius=0
     MSA=False
     ...
     if n.count_nonzero(connect) < data.shape[0] :
        data_connect_file = open('connected_data_{0}.dat', "w")
        for i in range(0,C.shape[0]) :
            if connect[i]==1 :
     ...
     #1
     tmp=1000000
     if(r_min>=0) : tmp=r_min
     elif(r_min==-1) : tmp=rmax-std
     ...
     D_file = open('D_residual_{0}.dat', "w")
     ...
     print('statistics_{0}.dat'.format(filename))
     stat_file= open(str('statistics_{0}.dat'.format(filename), "w"))
