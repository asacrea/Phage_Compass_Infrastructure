...
#Filtrar las taxonomias que están en la matrix filtrada
new_df = pd.DataFrame(columns=['Phage accession', 'Host lineage'])
for i in real_names:
    new_df = new_df.append(phago_bacteria.iloc[i])
    
df = df.fillna(1)

df.to_csv('/Users/andressacre/Documents/Universidad/Investigación_Thomas/Data/Data.tsv', sep='\t', header=False, index=False)
new_df.to_csv('/Users/andressacre/Documents/Universidad/Investigación_Thomas/Data/Taxonomies.tsv', sep='\t', index=False)
